...
- **Healthcare Information**: Learn about Hope Healthcare.
...
1. **Clone the repository:**
   ...
   ```bash
    git clone https://github.com/Kowshikreddy004/Hospital-Management-System.git
   ```
...
Copy `Backend/config/config.env.example` to `Backend/config/config.env` and replace every placeholder before starting the backend. The local config file is ignored by Git.
...
The backend creates the admin account from `ADMIN_EMAIL` and `ADMIN_PASSWORD` on startup when it does not already exist.
